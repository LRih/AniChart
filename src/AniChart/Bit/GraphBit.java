...
public final class GraphBit
{
    ...
    private void drawMinorXAxis(Graphics2D g)
    {
        ...
        int interval = (int)Math.max(_xValues.length / (width() / PIXELS_PER_INTERVAL), 1);
        float x;

        // draw left side
        int index = midIndex;

        while ((x = getX(index)) >= _x1)
        {
    ...
    private void drawMinorXAxis(Graphics2D g, FontMetrics metrics, String text, float x)
    {
        ...
        g.draw(new Line2D.Float(x, _y1, x, _y2 + MINOR_AXIS_EXTRA));

        g.setColor(Colors.TEXT);
        g.drawString(text, x - (float)rect.getCenterX(), _y2 + MINOR_AXIS_EXTRA - (float)rect.getY());
    }
    ...
    private void drawMinorYAxis(Graphics2D g)
    {
        ...
        int interval = (int)Math.max(Math.abs((_max - _min) / (height() / PIXELS_PER_INTERVAL)), 1);
        float y;
    ...
    private void drawMinorYAxis(Graphics2D g, FontMetrics metrics, String text, float y)
    {
        ...
        g.setColor(Colors.MINOR_AXIS);
        g.draw(new Line2D.Float(_x1 - MINOR_AXIS_EXTRA, y, _x2, y));

        g.setColor(Colors.TEXT);
        g.drawString(text, _x1 - MINOR_AXIS_EXTRA - (float)rect.getWidth(), y - (float)rect.getCenterY());
    }

    private void drawAxis(Graphics2D g)
    {
        // TODO replace with bit

        g.setColor(Colors.AXIS);
        g.setStroke(new BasicStroke(AXIS_WIDTH));

        g.draw(new Line2D.Float(_x1, _y1, _x1, _y2)); // vertical axis

        // constrain axis to chart bounds
        float y = getY(0);
        y = Math.min(Math.max(y, _y1), _y2);

        g.draw(new Line2D.Float(_x1, y, _x2, y)); // horizontal axis
    }
    ...
    public final float getX(int index)
    {
        ...
        return _x1 + width() / ((float)_xValues.length - 1) * index;
    }
    ...
    public final float getY(float value)
    {
        ...
        float top = _y1 + height() / 8;
        float bottom = _y2 - height() / 8;
